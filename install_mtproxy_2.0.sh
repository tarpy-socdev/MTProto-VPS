...
echo -e "${YELLOW}${BOLD}📌 Что такое спонсорский тег?${NC}"
echo ""
echo " Когда пользователь подключается к твоему прокси,"
echo " Telegram показывает плашку с названием канала"
echo " или именем — это спонсорский тег."
...
echo " 3. Введи данные прокси (см. ниже)"
echo " 4. Бот выдаст тег — вставь его сюда"
echo ""

read -rp " Ввести спонсорский тег? (y/n): " TAG_CHOICE

if [[ "$TAG_CHOICE" =~ ^[Yy]$ ]]; then
    read -rp " Введи тег: " SPONSOR_TAG
    if [ -n "$SPONSOR_TAG" ]; then
        # Обновляем systemd файл
